...
def matching_loss(struct_var):
    '''
    Loss is the negative number of elements in the matching that are located in the main diagonal
    The optimal matching will correspond to the identity matrix
...
def run_toy_example(args=None):
    ...
    for i in tqdm(range(args.iters)):
        if i % args.eval_every == 0:
            ...
            struct_var_mc = matching_struct(exp_mc).to(device)
            loss_value_mc = matching_loss(struct_var_mc)
        ...
        struct_var = matching_struct(exp).to(device)
        loss_value = matching_loss(struct_var)
